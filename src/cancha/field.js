...
// Carga el mapa de entorno HDR
new RGBELoader().load('/2k.hdr', (hdrTexture) => {
  hdrTexture.mapping = THREE.EquirectangularReflectionMapping
  scene.environment  = hdrTexture   // afecta todos los materiales PBR
  scene.background = hdrTexture
  scene.backgroundIntensity = 1.0  // ← ponlo en 1 para ver si aparece  // descomenta si quieres el HDR como fondo
})


  // Material de cristal con más visibilidad
  ...
  // Plano principal de cristal (105 × 68 m FIFA)
  const fieldMesh = new THREE.Mesh(
  new THREE.BoxGeometry(109, 72, 0.6),  // ← 7.5m extra por cada lado
  fieldMaterial
)
  ...
  new THREE.BoxGeometry(16, 16, 16),  // ← 7.5m extra por cada lado
  fieldMaterial2
)
  ...
  // Brillo tenue azul en la cara inferior
...
matFolder.addColor({ color: '#5599dd' }, 'color').name('Color').onChange(v => {
